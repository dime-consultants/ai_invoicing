...
def ai_job_list_create(request):
    """
    GET  /api/ai/jobs/
    ...
    if request.method == "GET":
        ...
        if batch_id := request.query_params.get("batch"):
            qs = qs.filter(batch_id=batch_id)
        if task_type := request.query_params.get("task_type"):
            qs = qs.filter(task_type=task_type)
        if job_status := request.query_params.get("status"):
            qs = qs.filter(status=job_status)
        return Response(AIAnalysisJobListSerializer(qs, many=True).data)
...
@api_view(["GET"])
@authenticated_required
def ai_insight_list(request, job_id):
    """
    ...
    if not job_qs.exists():
        return Response(AIInsightSerializer(AIInsight.objects.none(), many=True).data)

    qs = AIInsight.objects.filter(job_id=job_id).order_by("-severity", "created_at")

    if sev := request.query_params.get("severity"):
        qs = qs.filter(severity=sev)
    if itype := request.query_params.get("type"):
        qs = qs.filter(insight_type=itype)
    if actioned := request.query_params.get("actioned"):
...
@api_view(["POST"])
@authenticated_required
def ai_insight_action(request, pk):
    """
...
@api_view(["GET"])
@authenticated_required
def batch_ai_job_list(request, batch_id):
    """
...
def ai_analyze(request):
    ...
    try:
        ...
        logger.exception("ai_analyze failed: %s", exc)
        results = [
            {
                "id":         "fallback-1",
...
def _persist_as_chat_attachments(request, output_files, job_id, summary_text):
    ...
    download links pointing at chat/views.py:chat_attachment_download
    ...
    obj.file.url and would skip chat_attachment_download's ownership
...
def ai_run_analysis(request):
    ...
    chat/views.py:chat_attachment_download rather than a second download path.
    ...
    try:
        ...
        logger.exception("ai_run_analysis failed: %s", exc)
        return Response(
    ...
    if job_id:
        ...
        if output_files:
            output_files_meta = _persist_as_chat_attachments(
...
@api_view(["GET"])
@authenticated_required
def ai_recent_insights(request):
    """
