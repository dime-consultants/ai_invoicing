...
from .reconciliation_report import (
    is_reconciliation_result as _is_reconciliation_result,
    reconciliation_sections as _reconciliation_sections,
    build_reconciliation_xlsx as _build_reconciliation_xlsx,
    build_reconciliation_csv as _build_reconciliation_csv,
    build_reconciliation_pdf as _build_reconciliation_pdf,
)


def _reconciliation_title(tc) -> str:
    from .reconciliation_report import reconciliation_title
    ...
    if file_a_id == file_b_id and (sheet_a or sheet_b):
        return reconciliation_title(sheet_a, sheet_b)

    names = dict(
        UploadedFile.objects.filter(pk__in=[v for v in (file_a_id, file_b_id) if v])
        .values_list("pk", "original_filename")
    )
    return reconciliation_title(names.get(file_a_id), names.get(file_b_id))
