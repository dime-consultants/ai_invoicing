...
class CommunicationService:
    ...
    All application email is delivered exclusively through Resend.
    """
    ...
    def send_email(*, to: str, subject: str, message: str) -> bool:
        ...
        from_address = getattr(
            settings,
            "RESEND_FROM_ADDRESS",
            "noreply@dimeconsultants.africa",
        )
        ...
        try:
            ...
            logger.info(
                "Email sent through Resend to=%s subject=%s resend_id=%s",
                to,
                subject,
            ...
            logger.exception(
                "Resend email delivery failed to=%s subject=%s",
                to,
                subject,
            )
            return False
...
class OTPService:
    ...
    def issue(user: User, *, purpose: str) -> tuple[EmailOTP, str]:
        ...
        logger.info(
            "OTP issued user=%s purpose=%s otp_id=%s",
            user.pk,
            purpose,
            otp.pk,
        )
    ...
    def send(user: User, *, purpose: str) -> EmailOTP:
        ...
        if OTPService.uses_fixed_stage_code():
            logger.info(
                "Skipping OTP email in stage; fixed code active "
                "user=%s purpose=%s",
                user.pk,
                purpose,
            )
        ...
        delivered = CommunicationService.send_email(
            ...
            message=(
                ...
                f"{getattr(settings, 'EMAIL_OTP_TTL_MINUTES', 10)} minutes. "
        ...
        if not delivered:
            logger.warning(
                "OTP email not delivered user=%s purpose=%s",
                user.pk,
                purpose,
            )
    ...
    def verify(
        ...
        *,
        ...
        if not otp:
            logger.info(
                "OTP verify failed: no active OTP "
                "user=%s purpose=%s",
                user.pk,
                purpose,
            )
        ...
        if otp.is_expired:
            ...
            logger.info(
                "OTP verify failed: expired otp_id=%s user=%s",
                otp.pk,
                user.pk,
            )
        ...
        if otp.attempts >= otp.max_attempts:
            ...
            logger.warning(
                "OTP verify failed: attempts exceeded "
                "otp_id=%s user=%s",
                otp.pk,
                user.pk,
            )
