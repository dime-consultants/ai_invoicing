...
# ── Inline: files inside a batch ─────────────────────────────────────────────

class UploadedFileInline(admin.TabularInline):
    model   = UploadedFile
    extra   = 0
    fields  = (
        "original_filename", "extension", "file_size_bytes",
...
class UploadBatchAdmin(admin.ModelAdmin):
    list_display  = (
        "id", "label", "uploaded_by", "status_badge",
    ...
    fieldsets = (
        (None, {
            "fields": ("label", "description", "uploaded_by", "status"),
        }),
        ("Counts", {
...
class UploadedFileAdmin(admin.ModelAdmin):
    list_display  = (
        "original_filename", "extension", "batch",
    ...
    fieldsets = (
        ...
        ("Parsing", {
            "fields": ("parse_status", "parse_error", "parsed_at"),
        }),
        ("Extracted Content", {
    ...
    def parse_status_badge(self, obj):
        ...
        colour = colours.get(obj.parse_status, "#6c757d")
        return format_html(
            '<span style="background:{};color:#fff;padding:2px 8px;'
            'border-radius:4px;font-size:0.8em">{}</span>',
            colour,
