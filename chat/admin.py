...
class ChatMessageInline(admin.TabularInline):
    ...
    readonly_fields = ("role", "content_preview", "ai_job", "applied_workflow", "created_at")
...
class ChatMessageAttachmentInline(admin.TabularInline):
    ...
    extra   = 0
    fields  = (
        "filename", "file_type", "attachment_type",
...
class ChatConversationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            "fields": ("user", "title", "related_batch", "is_active"),
        }),
        ("Timestamps", {
...
class ChatMessageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Links", {
            "fields": ("ai_job", "applied_workflow"),
        }),
        ("Timestamps", {
            "fields": ("created_at",),
            "classes": ("collapse",),
...
class ChatMessageAttachmentAdmin(admin.ModelAdmin):
    list_display  = (
        "filename", "file_type", "attachment_type_badge",
    ...
    readonly_fields = (
        "message", "file", "filename", "file_type",
        "attachment_type", "file_size_bytes",
    ...
    def attachment_type_badge(self, obj):
        colours = {
            "user_upload":      "#0d6efd",
            "assistant_output": "#198754",
        }
        colour = colours.get(obj.attachment_type, "#6c757d")
        return format_html(
            '<span style="background:{};color:#fff;padding:2px 8px;'
            'border-radius:4px;font-size:0.8em">{}</span>',
            colour, obj.get_attachment_type_display(),
        )
...
class WorkflowAdmin(admin.ModelAdmin):
    list_display  = (
        "name", "workflow_type", "enabled_badge",
    ...
    fieldsets = (
        (None, {
            "fields": ("name", "description", "workflow_type", "enabled", "is_default"),
        }),
        ("Tool Steps", {
            "fields": ("steps",),
            "description": (
                "Ordered list of ToolDefinition.name values. "
                "The AI will use these tools (in order) when this workflow is active."
            ),
        }),
        ("Prompt Override", {
            "fields": ("system_prompt_prefix",),
            "classes": ("collapse",),
            "description": "Extra instruction injected into the system prompt when this workflow is selected.",
        }),
        ("Timestamps", {
            "fields": ("created_at", "updated_at"),
            "classes": ("collapse",),
